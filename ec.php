 ...
 * Only some of the user handleable error are rethrown as exceptions. Configured by EC_XXX defines below.
 ...
 */
function ec_error_handler($errno, $errmsg, $errfile, $errline, $errcontext)
{  
  ...
  ec_set_error_get_last($errno, $errmsg, $errfile, $errline, $errcontext);
...
function ec_re_error_log($errno, $errmsg, $errfile, $errline)
{
  ...
  if($errno & error_reporting())
  {
    ...
    if(ini_get("log_errors"))
    {
      ...
      {
          error_log(ec_make_error_log($errno, $errmsg, $errfile, $errline));
          $logged = true;
      }
    }
    ...
    if(ini_get("display_errors"))
    {
      if(ini_get("display_errors") == "stderr")
      {
        ...
        fwrite($f_out, ini_get("error_prepend_string").ec_make_error_log($errno, $errmsg, $errfile, $errline).ini_get("error_append_string"));
      }
      else
      {
        print ini_get("error_prepend_string").ec_make_error_log($errno, $errmsg, $errfile, $errline).ini_get("error_append_string");
      }
    }    
  }
  return $logged;
 ...
 * as of PHP 5.3.
 */
function ec_make_error_log($errno, $errmsg, $errfile, $errline)
{ 
 ...
 */
function ec_set_error_get_last($errno, $errmsg, $errfile, $errline, $errcontext, $was_exception = false)
{
